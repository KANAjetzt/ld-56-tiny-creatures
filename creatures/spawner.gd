...
func spawn() -> void:
	...
	if not local:
		...
		var random_x: int = (randf_range(spawn_offset_min, spawn_offset_max) + camera_view_size.x) * multiplier_x
		var multiplier_y := 1 if randf() > 0.5 else -1
		var random_y: int = (randf_range(spawn_offset_min, spawn_offset_max) + camera_view_size.y) * multiplier_y
	...
	for _habitat in creature.habitats:
		if Global.current_habitats.has(_habitat.id) and Global.current_habitats[_habitat.id].space_free > 0:
			Global.current_habitats[_habitat.id].remove()
			break
